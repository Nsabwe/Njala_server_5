...
// Fixed image upload to save Cloudinary URLs in MongoDB
// Keeps all real‑time features (Socket.IO, notifications, chat)
...
io.on('connection', (socket) => {
  ...
  socket.on('send_message', async (data) => {
    try {
      ...
      const chat = new ChatMessage({ senderId: userId, receiverId, message });
      await chat.save();
...
// ============================================================
//  LISTING ROUTES – FIXED
// ============================================================
// Helper to create listing with proper image handling
const createListing = async (Model, data, user, file) => {
  let imageUrl = '';

  // 1. If we have a valid file from multer (with buffer)
  if (file && file.buffer && file.buffer.length > 0) {
    imageUrl = await uploadImage(file.buffer);
  }
  // 2. Else if base64 data is provided (from cropping)
  else if (data.imageData && data.imageData.startsWith('data:image')) {
    const base64Data = data.imageData.replace(/^data:image\/\w+;base64,/, '');
    const buffer = Buffer.from(base64Data, 'base64');
    imageUrl = await uploadImage(buffer);
  }
  // If neither, imageUrl stays '' (no image)
...
app.put('/api/products/:id', authMiddleware, upload.single('image'), async (req, res) => {
  try {
    ...
    // Handle new image: file first, then base64
    if (req.file && req.file.buffer && req.file.buffer.length > 0) {
...
// Jobs (no image upload, unchanged)
...
app.post('/api/chat/messages', authMiddleware, async (req, res) => {
  try {
    ...
    const chat = new ChatMessage({ senderId: req.user._id, receiverId, message });
    await chat.save();
...
// ============================================================
//  NOTIFICATION CENTER API (unchanged)
// ============================================================
